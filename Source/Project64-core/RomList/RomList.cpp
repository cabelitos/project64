...
void CRomList::FillRomList(strlist & FileList, const char * Directory)
{
    ...
    WriteTrace(TraceRomList, TraceVerbose, "SearchPath: %s", Directory);
    ...
    do
    {
        WriteTrace(TraceRomList, TraceVerbose, "Found: \"%s\" m_StopRefresh = %s", Directory, m_StopRefresh ? "true" : "false");
        if (m_StopRefresh)
        {
